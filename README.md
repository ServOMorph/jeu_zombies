...
M0 à M4.4 sont validés. `python check.py` contrôle l'import, 23 suites headless, le franchissement réel d'une porte par un zombie et l'export `.pck`. La porte de sortie M3 est franchie après un retest ciblé ; la cause de l'échec initial (FPS minimum 28, compteur figé) n'a pas été formellement diagnostiquée. L'arsenal des six armes (M4.1), les achats muraux (M4.2), la caisse d'armes aléatoire de l'Entrepôt médical (M4.3) et la station d'amélioration du Laboratoire de synthèse (M4.4, ×1,35 dégâts) sont fonctionnels. M4.5 (quatre avantages, Accueil sécurisé) est implémentée et testée automatiquement, mais en attente de validation manuelle complète avant d'être cochée. Les phases DESIGN 1 à 5 sont validées : kit structurel, matériaux, signalétique, 13 accessoires, cinq zones visuelles, zombie standard skinné avec huit clips et arsenal FPS de sept silhouettes. Aucun de ces exports n'est encore intégré au jeu jouable.
