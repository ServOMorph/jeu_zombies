...
M0 à M4.5 sont validés (porte de sortie M4 franchie). `python check.py` contrôle l'import, 23 suites headless, le franchissement réel d'une porte par un zombie et l'export `.pck`. M5.1 est implémentée et testée automatiquement, mais attend la validation manuelle de son HUD. Les phases DESIGN 1 à 7 sont validées ; la phase 8 reste en attente de validation visuelle dans le laboratoire. Les assets low-poly restent les placeholders du prototype, tandis qu’une direction réaliste optimisée est engagée pour les futurs assets finaux avec deux planches de zombie produites. Les phases 9 et 10 restent préparatoires et aucun asset DESIGN ni audio final n’est intégré au jeu jouable.
