...
M0 à M5 sont validés (jalon M5 — Quête, finale et fins de partie — intégralement clos). Jalon M6 (Menus, options, présentation et audio) en cours, piloté par `roadmap_m6.md`. Phase P1 en cours : la zone pilote `couloirs` est murée avec le kit modulaire importé (pose data-driven réutilisable dans `world/zone_walls.gd`, collision de mur découplée des modules décoratifs), navmesh mesurée sans régression, contrôle visuel validé en jeu réel. Reste en attente avant de clore P1 : contrôle manuel FPS/appels de rendu listé dans `tests_manuels.md`. Audit REVIEW : Phases 1 et 2 terminées (20 findings). Phase 3 (Frontière développement/production) terminée, 5 findings supplémentaires ajoutés (2 BUG, 1 DETTE, 2 NETTOYAGE) — dont un point critique : le build release démarre actuellement sur la scène de développement et expose des hotkeys de triche non gardées.
