...
M0 à M4.5 sont validés (porte de sortie M4 franchie). `python check.py` contrôle l'import, 23 suites headless, le franchissement réel d'une porte par un zombie et l'export `.pck`. La porte de sortie M3 est franchie après un retest ciblé ; la cause de l'échec initial (FPS minimum 28, compteur figé) n'a pas été formellement diagnostiquée. L'arsenal des six armes (M4.1), les achats muraux (M4.2), la caisse d'armes aléatoire de l'Entrepôt médical (M4.3), la station d'amélioration du Laboratoire de synthèse (M4.4, ×1,35 dégâts) et les quatre avantages de l'Accueil sécurisé (M4.5) sont fonctionnels et validés. M5.1 (machine d'état de quête, `QuestController`, neuf états séquentiels) est implémentée et testée automatiquement, mais en attente de validation manuelle de l'affichage HUD avant d'être cochée. Les phases DESIGN 1 à 7 sont validées ; la phase 8 est en cours avec 18 effets documentés et trois écrans de prévisualisation accessibles par `F4` dans le laboratoire DESIGN. Aucun export DESIGN n'est encore intégré au jeu jouable ; la validation visuelle phase 8 est en attente.
