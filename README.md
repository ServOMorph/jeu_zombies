...
M0 à M5 sont validés (jalon M5 — Quête, finale et fins de partie — intégralement clos). Jalon M6 (Menus, options, présentation et audio) en cours, piloté par `roadmap_m6.md`. Phase P1 en cours : la zone pilote `couloirs` est murée avec le kit modulaire importé (pose data-driven réutilisable dans `world/zone_walls.gd`, collision de mur découplée des modules décoratifs), navmesh mesurée sans régression. Reste en attente avant de clore P1 : contrôles manuels (z-fighting, FPS) listés dans `tests_manuels.md`.
