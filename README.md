...
Le niveau 1 Helix-9 reste disponible. Le niveau 2 Port est fonctionnel : vagues, extraction, progression persistante, stations et menu F3 de débogage. Le joueur commence au centre, dans une zone à quatre portes payantes et huit fenêtres d'apparition ; la première manche compte 20 zombies. Le Port inclut un délai de récupération d'endurance, une animation de rechargement et un panneau F3 déplaçable. Le zombie Mixamo Zombiegirl est animé, orienté par son déplacement, ancré au sol et applique ses dégâts 0,22 s après le début de la frappe. `python test.py` réussit (33 suites). La qualification manuelle du Port et du zombie reste à exécuter ; voir `tests_manuels.md`.
