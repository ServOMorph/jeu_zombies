...
M0 à M4.5 sont validés (porte de sortie M4 franchie). Le chantier DI est clos sur son périmètre administratif (34 designs importés et validés, 17 exports FPS exclus `a_revoir`) ; l'intégration visuelle en scène (tuilage du kit modulaire, mesh du zombie) est reportée au jalon M6.4. M5.1 (machine d'état de quête) et M5.2 (composants d'antidote, fabrication) sont implémentées, couvertes par tests automatisés et validées manuellement en jeu réel. M5.3 (déploiement de l'antidote et terminal d'extraction) est implémentée et couverte par tests automatisés ; la validation manuelle en jeu réel reste en attente (scénario dans `tests_manuels.md`).
