...
M0 à M4.5 sont validés (porte de sortie M4 franchie). Le chantier DI est clos sur son périmètre administratif (34 designs importés et validés, 17 exports FPS exclus `a_revoir`) ; l'intégration visuelle en scène (tuilage du kit modulaire, mesh du zombie) est reportée au jalon M6.4. M5.1 (machine d'état de quête) et M5.2 (composants d'antidote, fabrication) sont implémentées et couvertes par tests automatisés (`python check.py` : 25 suites, aucune erreur). Validation manuelle de M5.2 en attente dans `tests_manuels.md`. Prochaine étape : M5.3 (déploiement de l'antidote et terminal d'extraction).
